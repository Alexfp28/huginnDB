...
| **Managed policy — phase 1: the AI** | Unreleased (next minor) | The objection an enterprise buyer raised: configuring and auditing permissions workstation by workstation. An administrator writes one policy — `HKLM\SOFTWARE\Policies\HuginnDB` or `managed-policy.json` in the system folder, inline or pointing to a share — giving each OS account one role and each role per-server rules: visible databases and relations, and select / insert / update / delete / ddl granted separately, plus `monitor`. Enforced for the AI at the one function every AI request passes through, with discovery filtered, free SQL withheld under scoped rules, `ai ⊆ human`, fail-closed, and only ever narrowing the local settings; Settings → Policy shows what applies. Preceded by a classifier fix (three writes that passed as reads) and the INSERT/UPDATE/DELETE verb split. Phase 2 applies it to **people** as a guardrail: every command checks the role's `human` permissions, and the interface locks what they do not allow, with the reason (gotchas #95–#96). Phase 3 makes it a wall: each person signs in with their own database user (a rule's `dbUser`), and HuginnDB generates the `GRANT` / `createRole` script that makes the database agree with the role (gotchas #97–#98). Phase 4 edits it in the app: a form and the JSON over one draft, checked by the parser that applies it, with "view as", saved only where the share accepts a write (gotchas #99–#100). See `docs/POLICY.md`, `docs/POLICY_ROADMAP.md` and gotchas #93–#100. |
...
- The heaviest single files were `DocumentListView.tsx` (6 raw buttons, 4 OS
  ...
  **`ConnectionTreeRow.tsx` is down to 3**, done while adding the `failed`
  match state below: `MatchBadge`'s six arms became one `SimpleTooltip`, and
  the reason they were worth migrating rather than counted is that every one of
  them explains a state its glyph cannot carry — "—" means four different
  things across those branches — so the label *is* the badge, and an OS tooltip
  is the wrong vehicle for it. The three left are the row's own controls.
  `DocumentListView.tsx` is still open, and its raw buttons are the documented
  exception, not work.
...
initialized }` — no concept of a slice whose read **failed**. So a connection
whose schema could not be loaded was `initialized: true, loading: false,
tables: []`, which `rowMatchState` read as `"none"`: the row dimmed, grew a
confident `0` badge, and the filter *folded* it away — unmounting the one place
`cs.error` was rendered. In a multi-DB tree the same thing happened one level
down: a child whose warm failed is no longer "cold", so it dropped out of what
the warm button offers and was reported as an honest zero. That is precisely
the failure mode the header of `treeMatches.ts` and `MatchBadge`'s doc were
written to argue against — saying `0` about something nobody read is what makes
a user abandon a search — and, like the environment switch before it, the tree
could not act on a fact the store already held. Found while fixing gotcha #68
and closed by a `failed` member of `RowMatchState`, ranked above `unloaded`
(looking did not work) and below `matches` (a stale count is still a count),
plus the `error` field on the slice shape to back it. The two places it is
