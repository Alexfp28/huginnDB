...
11. **`sqlx` rejects `try_get::<Vec<u8>>` on a MySQL `BIT` column.** Its blob type-compatibility check only accepts BLOB / STRING / VARBINARY, not `MYSQL_TYPE_BIT`, so the call returns `Err` and the cell collapses to `Value::Null` (grid shows "NULL" for a real value). Decode BIT by reading `raw.as_bytes()` straight off the `ValueRef` (`mysql_value` in `src-tauri/src/db/values.rs`), folding big-endian into an integer. Also: in `mysql_value` the `BOOL` / `TINYINT(1)` branch must stay **above** `name.contains("INT")` — `"TINYINT(1)"` contains `"INT"`, so the order matters.
