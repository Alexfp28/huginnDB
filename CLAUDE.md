...
- 1.4.0 (in progress) removed workspaces in favour of native per-window instances — "New window" in the new **Window** menu opens a blank, ephemeral secondary window; see gotcha #8 and `docs/1.4.0_ROADMAP.md`. Same cycle also split the topbar File/View menus into four (File/Window/View/Help — File had accumulated unrelated window/help actions) and fixed two bugs: `open_new_window` must be an `async fn` on Windows (sync commands deadlock creating a `WebviewWindow`, a WebView2 issue) and CLI ad-hoc launches without `--password` now always attempt the connect instead of silently staying disconnected. Server-side users/permissions introspection (roadmap item 2) is still open.
