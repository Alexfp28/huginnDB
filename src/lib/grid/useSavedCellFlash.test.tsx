...
describe("marking a saved cell", () => {
  it("records the cell that just saved", () => {
    ...
    expect(result.current.flashed).toEqual({ rowKey: "id=1", path: ["email"] });
  });

  it("ignores an empty path", () => {
    // Nothing on screen to point at.
    const { result } = renderHook(() => useSavedCellFlash());
    act(() => result.current.markSaved("id=1", []));
    expect(result.current.flashed).toBeNull();
  });

  it("keeps a nested path whole", () => {
    // The two readers disagree about granularity on purpose: the table view
    // pulses the `customData` cell, the list view the `format` field.
    const { result } = renderHook(() => useSavedCellFlash());
    act(() => result.current.markSaved("id=1", ["customData", "format"]));
    expect(result.current.flashed?.path).toEqual(["customData", "format"]);
  });
...
describe("a second save re-arms the pulse", () => {
  it("moves to the new cell", () => {
    ...
    expect(result.current.flashed).toEqual({ rowKey: "id=2", path: ["name"] });
  });
