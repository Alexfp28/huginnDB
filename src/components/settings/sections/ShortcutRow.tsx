  ...
  return (
    ...
    >
      ...
      <div className="flex max-w-[20rem] flex-wrap items-center justify-end gap-1">
        <span
          className="rounded-sm border border-border/60 px-1 py-0.5 font-mono text-3xs uppercase tracking-wide text-muted-foreground/70"
        ...
        {bindings.length === 0 && (action.fixed ?? []).length === 0 && (
          <button
            type="button"
            onClick={() => onEdit(null)}
            className="rounded-sm border border-dashed border-border px-1.5 py-0.5 font-mono text-2xs text-muted-foreground hover:border-brand hover:text-foreground"
          >
        ...
        {action.fixed?.map((binding) => (
          <span
            ...
            className="rounded-sm border border-border/60 px-1.5 py-0.5 font-mono text-2xs text-muted-foreground/50"
          >
            {formatForDisplay(binding)}
          </span>
        ))}
