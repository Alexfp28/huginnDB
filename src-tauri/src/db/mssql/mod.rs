...
/// Upper bound on concurrent TDS sessions per connection.
///
/// Matches the spirit of `MAX_CONNECTIONS_SERVER` in [`crate::db::pool`]: a
/// desktop client rarely has more than a couple of statements in flight (a
/// query, a schema-tree refresh, the keepalive ping), and each session is a
/// real server-side connection.
...
struct Inner {
    ...
    /// Sessions that are open but not in use. Never longer than
    /// `MAX_SESSIONS`, because a permit is required to take one out.
...
impl MsSqlPool {
    ...
    pub async fn ping(&self) -> AppResult<()> {
        ...
        c.simple_query_sets("SELECT 1").await.map(|_| ())
    }
